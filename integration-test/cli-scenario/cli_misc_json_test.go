...
func TestCliMiscJson(t *testing.T) {
	t.Run("command misc json in/out test for mock driver", func(t *testing.T) {
		...
		tc = TestCases{
			...
			CmdArgs: []string{"credential", "create", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "-d",
				`{
					...
					"ProviderName":"MOCK",
					"KeyValueInfoList": [
						{"Key":"MockName", "Value":"mock_unit_misc_json"}
					]
				}`,
			},
		...
		tc = TestCases{
			Name: "create image",
			CmdArgs: []string{"image", "create", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "-d",
				`{
		...
		tc = TestCases{
			Name:                "list image",
			CmdArgs:             []string{"image", "list", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01"},
		...
		tc = TestCases{
			Name:                "get image",
			CmdArgs:             []string{"image", "get", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--name", "cirros-0.5.1"},
			ExpectResStartsWith: `{"IId":{"NameId":"cirros-0.5.1","SystemId":"cirros-0.5.1"}`,
		}
		SpiderCmdTest(t, tc)
		...
		tc = TestCases{
			Name:                "list vmspec",
			CmdArgs:             []string{"vmspec", "list", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01"},
		...
		tc = TestCases{
			Name:                "get vmspec",
			CmdArgs:             []string{"vmspec", "get", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--name", "mock-vmspec-01"},
		...
		tc = TestCases{
			...
			CmdArgs:             []string{"vmspec", "listorg", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01"},
			ExpectResStartsWith: "\n",
		}
		SpiderCmdTest(t, tc)
		...
		tc = TestCases{
			Name:                "get org vmspec",
			CmdArgs:             []string{"vmspec", "getorg", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--name", "org-vmspec-01"},
			ExpectResStartsWith: "\n",
		}
		SpiderCmdTest(t, tc)
		...
		tc = TestCases{
			Name:                "list all vpc",
			CmdArgs:             []string{"vpc", "listall", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01"},
		...
		tc = TestCases{
			...
			CmdArgs:             []string{"security", "listall", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01"},
		...
		tc = TestCases{
			...
			CmdArgs:             []string{"keypair", "listall", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01"},
		...
		tc = TestCases{
			Name:                "list all vm",
			CmdArgs:             []string{"vm", "listall", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01"},
		...
		tc = TestCases{
			...
			CmdArgs:             []string{"vm", "terminatecsp", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--id", "vm-01"},
		...
		tc = TestCases{
			...
			CmdArgs:             []string{"keypair", "deletecsp", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--id", "keypair-01"},
		...
		tc = TestCases{
			...
			CmdArgs:             []string{"security", "deletecsp", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--id", "vpc-01-deli-sg-01"},
			ExpectResStartsWith: `{"Result":true}`,
		}
		SpiderCmdTest(t, tc)
		...
		tc = TestCases{
			...
			CmdArgs:             []string{"vpc", "removecsp-subnet", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--vname", "vpc-01", "--id", "subnet-01"},
			ExpectResStartsWith: `{"Result":true}`,
		}
		SpiderCmdTest(t, tc)
		...
		tc = TestCases{
			Name:                "delete csp vpc",
			CmdArgs:             []string{"vpc", "deletecsp", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--id", "vpc-01"},
			ExpectResStartsWith: `{"Result":true}`,
		}
		SpiderCmdTest(t, tc)
		...
		tc = TestCases{
			Name:                "delete image",
			CmdArgs:             []string{"image", "delete", "--config", "../conf/grpc_conf.yaml", "-i", "json", "-o", "json", "--cname", "mock-unit-config01", "--name", "cirros-0.5.1"},
			ExpectResStartsWith: `{"Result":true}`,
		}
		SpiderCmdTest(t, tc)
